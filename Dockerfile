...
### NGINX2
FROM nginx:${NGINX_VERSION}-alpine AS symfony_docker_nginx2

COPY docker/nginx/conf.d/default2.conf /etc/nginx/conf.d/default.conf
COPY public /srv/app/public/
...
RUN set -eux \
	...
	&& git clone --single-branch --branch add-jaeger-support https://github.com/kraynel/dd-trace-php.git /etc/dd-trace-php \
	&& cd /etc/dd-trace-php \
	&& phpize \ 
